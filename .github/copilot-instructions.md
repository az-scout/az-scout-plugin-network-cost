# Copilot Instructions for az-scout-plugin-network-cost
...
├── routes.py            # FastAPI APIRouter (mounted at /plugins/network-cost/)
├── tools.py             # MCP tool functions (exposed on the az-scout MCP server)
└── static/
    ├── css/
    │   └── network-cost.css      # Plugin styles (auto-loaded via css_entry)
    ├── html/
    │   └── network-cost-tab.html # HTML fragment (fetched by JS at runtime)
    └── js/
        └── network-cost-tab.js   # Tab UI logic (auto-loaded via js_entry)
```
...
network_cost = "az_scout_network_cost:plugin"
```
